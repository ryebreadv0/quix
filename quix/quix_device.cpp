...
class device_impl
{
  ...
  private:
    void create_instance(const char *app_name, uint32_t app_version, const char *engine_name, uint32_t engine_version);
    void create_window(const char* app_name);
    void create_surface();
    ...
    static constexpr std::array<const char *, 1> requested_extensions = { VK_KHR_SWAPCHAIN_EXTENSION_NAME };
...
{
    m_logger.add_sink(std::make_shared<spdlog::sinks::basic_file_sink_mt>("logs/device.log", true));
...
void device_impl::create_window(const char* app_name) {
    ...
    window = glfwCreateWindow(800, 600, "Quix", nullptr, nullptr);
...
{
    VkResult error;
    if ((error = glfwCreateWindowSurface(m_instance, window, nullptr, &m_surface)) != VK_SUCCESS) {
...
queue_family_indices device_impl::find_queue_families(VkPhysicalDevice physical_device)
{
    if (m_queue_family_indices.has_value()) { return m_queue_family_indices.value(); }
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(physical_device, &queueFamilyCount, queueFamiliesProperties.data());
	...
	vkGetPhysicalDeviceSurfaceSupportKHR(physical_device, iterator, m_surface, &presentSupport);
	if (presentSupport != 0U) { indices.present_family = iterator; }
	...
	// m_queue_family_indices = indices; // TODO this needs to be set after the device is selected
	return indices;
    } else {
...
bool device_impl::check_device_extension_support(VkPhysicalDevice physical_device)
{
    ...
    for (const auto &ext : requested_extensions) { requiredExtensions.insert(ext); }
...
swapchain_support_details device_impl::query_swapchain_support(VkPhysicalDevice physical_device)
{
	...
	vkGetPhysicalDeviceSurfacePresentModesKHR(
	  physical_device, m_surface, &presentModeCount, details.present_modes.data());
    }
...
int device_impl::get_supported_feature_score(VkPhysicalDevice physical_device)
{
    ...
    if (requested_features == nullptr) { return features_score; }
...
int device_impl::rate_physical_device(VkPhysicalDevice physical_device)
{
    ...
    score +=
      static_cast<int>((static_cast<double>(deviceMemory) / memory_score_multiplier * memory_score_divider) + 0.5);
...
void device_impl::pick_physical_device()
{
    ...
    auto *devices = static_cast<VkPhysicalDevice *>(alloca(sizeof(VkPhysicalDevice) * deviceCount));
    ...
    if (m_physical_device == VK_NULL_HANDLE) { throw std::runtime_error("failed to find a suitable GPU!"); }
}


// device class
device::device(const char *app_name, uint32_t app_version)
  : m_impl(std::make_unique<device_impl>(app_name, app_version, "quix", VK_MAKE_VERSION(1, 0, 0), this))
{}

device::~device() = default;

NODISCARD GLFWwindow* device::window() const noexcept { 
